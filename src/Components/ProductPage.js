...
useEffect(() => {
  ...
  const getProductById = async () => {
    try {
      // Usa o 'id' da URL para buscar o produto específico
      const response = await fetch(`https://fakestoreapi.com/products/${id}`);
      if (!response.ok) {
        throw new Error("Produto não encontrado");
      }
      const productData = await response.json();
      setProduct(productData); // Agora 'productData' é um único objeto

    } catch (error) {
      console.error("Erro ao buscar o produto:", error);
      setProduct(null); // Define como null em caso de erro para poder tratar na UI
    }
  };

  // Garante que a busca só aconteça se houver um 'id'
  if (id) {
    getProductById();
  }

  // Gera o número de reviews
